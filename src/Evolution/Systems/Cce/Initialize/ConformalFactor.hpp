...
namespace ConformalFactor_detail {
...
struct ConformalFactor<false> : InitializeJ<false> {
  using options = ConformalFactor_detail::options;
  static constexpr Options::String help = {
...
/*!
 * \brief Generate initial data with a chosen angular conformal factor (as
 * `ConformalFactor<false>`), additionally producing the inertial (partially
 * flat) coordinates required to evolve the partially flat Bondi-like
 * coordinates (`evolve_ccm = true`).
 *
 * \details Identical to `ConformalFactor<false>` for the Cauchy angular solve,
