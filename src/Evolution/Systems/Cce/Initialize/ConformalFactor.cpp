...
void read_modes_from_input_file(
  ...
  for (size_t i = 0; i < input_modes->size(); ++i) {
    (*input_modes)[i] = std::complex<double>(
        initial_j_modes(0, 2 * i), initial_j_modes(0, 2 * i + 1));  // NOLINT
  }
}
...
std::unique_ptr<InitializeJ<true>> ConformalFactor<true>::get_clone() const {
  return std::make_unique<ConformalFactor<true>>();
}
std::unique_ptr<InitializeJ<false>> ConformalFactor<false>::get_clone() const {
  return std::make_unique<ConformalFactor<false>>();
}

// --- Added migration constructors to satisfy the declaration in the header ---
// migration constructors (required by PUP::able/Charm) were declared in the
// header but not defined; define them to forward to the base InitializeJ
...
void ConformalFactor<true>::operator()(
  ...
  Variables<tmpl::list<::Tags::TempSpinWeightedScalar<0, 2>,
                       ...
                       ::Tags::TempSpinWeightedScalar<16, 0>,
                       ::Tags::TempSpinWeightedScalar<17, 2>,
                       ::Tags::TempSpinWeightedScalar<18, 2>,
                       ::Tags::TempSpinWeightedScalar<19, 0>>>
  ...
  auto& surface_beta_buffer =
      get<::Tags::TempSpinWeightedScalar<16, 0>>(buffers);
  auto& surface_j_buffer_inv =
      get<::Tags::TempSpinWeightedScalar<17, 2>>(buffers);
  auto& surface_dr_j_buffer_inv =
      get<::Tags::TempSpinWeightedScalar<18, 2>>(buffers);
  auto& input_j_buffer =
  ...
  auto& surface_r_buffer_inv =
      get<::Tags::TempSpinWeightedScalar<19, 0>>(buffers);
  auto& gauge_omega_inv = get<::Tags::TempSpinWeightedScalar<12, 0>>(buffers);
  auto& filtered_gauge_omega_inv =
      get(get<::Tags::TempSpinWeightedScalar<13, 0>>(buffers));
  auto& target_omega_inv =
      get(get<::Tags::TempSpinWeightedScalar<14, 0>>(buffers));
  auto& interpolated_target_gauge_omega_inv =
      get(get<::Tags::TempSpinWeightedScalar<15, 0>>(buffers));
  ...
  Variables<tmpl::list<::Tags::ModalTempSpinWeightedScalar<0, 2>,
                       ...
                       ::Tags::ModalTempSpinWeightedScalar<2, 0>>>
  ...
  auto& gauge_omega_transform_buffer_inv =
      get(get<::Tags::ModalTempSpinWeightedScalar<2, 0>>(modal_buffers));
  ...
  if (use_input_modes_) {
    if (input_mode_filename_.has_value()) {
      ...
      std::fill(goldberg_modes.data().begin(), goldberg_modes.data().end(),
                0.0);
      std::copy(input_modes_.begin(), input_modes_.end(),  // NOLINT
  ...
  auto finalize_function =
      [&gauge_omega, &l_max, &surface_dr_j_buffer, &boundary_dr_j, &boundary_j,
       &surface_j_buffer, &surface_r_buffer, &r, &surface_beta_buffer,
       &target_omega_inv](
          const Scalar<SpinWeighted<ComplexDataVector, 2>>& gauge_c,
          const Scalar<SpinWeighted<ComplexDataVector, 0>>& gauge_d,
        ...
        GaugeAdjustedBoundaryValue<Tags::BondiBeta>::apply(
            make_not_null(&surface_beta_buffer), r, gauge_omega, interpolator);
        interpolator.interpolate(make_not_null(&target_omega_inv),
                                 get(gauge_omega));
        target_omega_inv.data() = 1.0 / target_omega_inv.data();
      };
  ...
  auto iteration_function_inv =
      [&iteration_heuristic_function, &filtered_gauge_omega_inv,
       &gauge_omega_inv, &target_omega_inv,
       &interpolated_target_gauge_omega_inv, &gauge_omega_transform_buffer_inv,
       &l_max, this](
          const gsl::not_null<Scalar<SpinWeighted<ComplexDataVector, 2>>*>
              gauge_c_step_inv,
          const gsl::not_null<Scalar<SpinWeighted<ComplexDataVector, 0>>*>
              gauge_d_step_inv,
          const Scalar<SpinWeighted<ComplexDataVector, 2>>& gauge_c_inv,
          const Scalar<SpinWeighted<ComplexDataVector, 0>>& gauge_d_inv,
          const Spectral::Swsh::SwshInterpolator& iteration_interpolator_inv) {
        get(gauge_omega_inv).data() =
            0.5 * sqrt(get(gauge_d_inv).data() * conj(get(gauge_d_inv).data()) -
                       get(gauge_c_inv).data() * conj(get(gauge_c_inv).data()));
        iteration_interpolator_inv.interpolate(
            make_not_null(&interpolated_target_gauge_omega_inv),
            target_omega_inv);

        filtered_gauge_omega_inv = get(gauge_omega_inv);
        if (not optimize_l_0_mode_) {
          Spectral::Swsh::filter_swsh_boundary_quantity(
              make_not_null(&filtered_gauge_omega_inv), l_max, 1_st, l_max,
              make_not_null(&gauge_omega_transform_buffer_inv));
          Spectral::Swsh::filter_swsh_boundary_quantity(
              make_not_null(&interpolated_target_gauge_omega_inv), l_max, 1_st,
              l_max, make_not_null(&gauge_omega_transform_buffer_inv));
        }
        double max_error = max(abs(filtered_gauge_omega_inv.data() -
                                   interpolated_target_gauge_omega_inv.data()));
        iteration_heuristic_function(
            make_not_null(&get(*gauge_c_step_inv)),
            make_not_null(&get(*gauge_d_step_inv)), get(gauge_omega_inv),
            filtered_gauge_omega_inv, interpolated_target_gauge_omega_inv,
            get(gauge_c_inv), get(gauge_d_inv), l_max);
        return max_error;
      };

  auto finalize_function_inv =
      [&gauge_omega_inv, &l_max, &surface_dr_j_buffer_inv, &surface_dr_j_buffer,
       &surface_j_buffer, &surface_j_buffer_inv, &surface_r_buffer_inv,
       &surface_r_buffer](
          const Scalar<SpinWeighted<ComplexDataVector, 2>>& gauge_c_inv,
          const Scalar<SpinWeighted<ComplexDataVector, 0>>& gauge_d_inv,
          const tnsr::i<DataVector, 2, ::Frame::Spherical<::Frame::Inertial>>&
          /*angular_cauchy_coordinates*/,
          const Spectral::Swsh::SwshInterpolator& interpolator_inv) {
        get(gauge_omega_inv).data() =
            0.5 * sqrt(get(gauge_d_inv).data() * conj(get(gauge_d_inv).data()) -
                       get(gauge_c_inv).data() * conj(get(gauge_c_inv).data()));
        GaugeAdjustedBoundaryValue<Tags::Dr<Tags::BondiJ>>::apply(
            make_not_null(&surface_dr_j_buffer_inv), surface_dr_j_buffer,
            surface_j_buffer, gauge_c_inv, gauge_d_inv, gauge_omega_inv,
            interpolator_inv, l_max);
        GaugeAdjustedBoundaryValue<Tags::BondiJ>::apply(
            make_not_null(&surface_j_buffer_inv), surface_j_buffer, gauge_c_inv,
            gauge_d_inv, gauge_omega_inv, interpolator_inv);
        GaugeAdjustedBoundaryValue<Tags::BondiR>::apply(
            make_not_null(&surface_r_buffer_inv), surface_r_buffer,
            gauge_omega_inv, interpolator_inv);
      };

  detail::iteratively_adapt_angular_coordinates(
      cartesian_inertial_coordinates, angular_inertial_coordinates, l_max,
      angular_coordinate_tolerance_, max_iterations_, 1.0e-2,
      iteration_function_inv, require_convergence_, finalize_function_inv);

  for (size_t i = 0; i < number_of_radial_points; i++) {
    Parallel::printf(
        "1e6 * boundary_j vs surface_j_inv vs surface_j %16f, %16f, %16f\n",
        1e6 * get(boundary_j).data()[i].real(),
        1e6 * get(surface_j_buffer_inv).data()[i].real(),
        1e6 * get(surface_j_buffer).data()[i].real());
  }
