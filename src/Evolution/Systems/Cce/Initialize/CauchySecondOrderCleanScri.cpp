...
#include "Evolution/Systems/Cce/Initialize/CauchySecondOrderCleanScri.hpp"
...
CauchySecondOrderCleanScri::CauchySecondOrderCleanScri(
...
std::unique_ptr<InitializeJ<false>> CauchySecondOrderCleanScri::get_clone()
    const {
  return std::make_unique<CauchySecondOrderCleanScri>(*this);
}

void CauchySecondOrderCleanScri::operator()(
  ...
  // Clean-scri variant: remove the residual scri+ value and second radial
  // derivative of J that survive the gauge transform. Writing the volume J as a
  // series in (1 - y), the constant term equals J at scri+ and the coefficient
  // of (1 - y)^2 equals half the second radial derivative there. Subtracting
  // both contributions across the full radial profile forces J and dy^2 J to
  // vanish exactly at scri+ while leaving the (1 - y) and (1 - y)^3 behaviour
  // untouched. We copy the scri+ slices first because the loop overwrites the
  // scri+ shell of J as it runs.
  const SpinWeighted<ComplexDataVector, 2> scri_j_view;
  make_const_view(make_not_null(&scri_j_view), get(*j),
                  (number_of_radial_points - 1) * number_of_angular_points,
                  number_of_angular_points);
  // NOLINTNEXTLINE(performance-unnecessary-copy-initialization)
  const ComplexDataVector scri_j_value = scri_j_view.data();
  const ComplexDataVector half_scri_dy_dy_j = 0.5 * scri_dy_dy_j.data();
  for (size_t i = 0; i < number_of_radial_points; ++i) {
    ComplexDataVector angular_view_j{
        get(*j).data().data() + number_of_angular_points * i,  // NOLINT
        number_of_angular_points};
    angular_view_j -=
        scri_j_value + square(one_minus_y_collocation[i]) * half_scri_dy_dy_j;
  }
}

void CauchySecondOrderCleanScri::pup(PUP::er& p) {
...
PUP::able::PUP_ID CauchySecondOrderCleanScri::my_PUP_ID =
    0;  // NOLINT  // NOLINT
}  // namespace Cce::InitializeJ
