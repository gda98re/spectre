...
namespace ZeroNonSmooth_detail {
 ...
 */
template <>
struct ZeroNonSmooth<false> : InitializeJ<false> {
  using AngularCoordinateTolerance =
      ZeroNonSmooth_detail::AngularCoordinateTolerance;
  using MaxIterations = ZeroNonSmooth_detail::MaxIterations;
  using RequireConvergence = ZeroNonSmooth_detail::RequireConvergence;
...
/*!
 * \brief Initialize \f$J\f$ on the first hypersurface to be vanishing
 * (as `ZeroNonSmooth<false>`), additionally producing the inertial (partially
 * flat) coordinates required to evolve the partially flat Bondi-like
 * coordinates (`evolve_ccm = true`).
 *
 * \details Identical to `ZeroNonSmooth<false>` for the Cauchy angular solve,
 * but also computes the inertial angular coordinates that invert the Cauchy
 * angular-coordinate transformation, via `detail::invert_angular_coordinates`.
 */
template <>
struct ZeroNonSmooth<true> : InitializeJ<true> {
  using AngularCoordinateTolerance =
      ZeroNonSmooth_detail::AngularCoordinateTolerance;
  using MaxIterations = ZeroNonSmooth_detail::MaxIterations;
  using RequireConvergence = ZeroNonSmooth_detail::RequireConvergence;
