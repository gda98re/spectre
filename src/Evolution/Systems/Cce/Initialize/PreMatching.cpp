...
#include "Evolution/Systems/Cce/Initialize/PreMatching.hpp"
...
PreMatching::PreMatching(const double angular_coordinate_tolerance,
...
std::unique_ptr<InitializeJ<false>> PreMatching::get_clone() const {
  return std::make_unique<PreMatching>(*this);
}

void PreMatching::operator()(
  ...
  // Initialize J in the original gauge by J = J0 + J1 / r matching
  auto& one_over_r_coeff = -square(get(r)) * get(boundary_dr_j);
  auto& constant_coeff = get(boundary_j) - one_over_r_coeff / get(r);
...
void PreMatching::pup(PUP::er& p) {
  p | require_convergence_;
  p | angular_coordinate_tolerance_;
  p | max_iterations_;
}

PUP::able::PUP_ID PreMatching::my_PUP_ID = 0;
}  // namespace Cce::InitializeJ
