 ...
 *
 * \note This is the "clean-scri" variant: after the gauge transform, the
 * residual scri+ value and second radial derivative of \f$J\f$ are subtracted
 * across the full radial profile (the constant and \f$(1 - y)^2\f$ terms of the
 * series in \f$(1 - y)\f$), so that both \f$J\f$ and \f$\partial_y^2 J\f$
 * vanish exactly at scri+.
 */
struct CauchySecondOrderCleanScri : InitializeJ<false> {
  ...
  WRAPPED_PUPable_decl_template(CauchySecondOrderCleanScri);  // NOLINT
  explicit CauchySecondOrderCleanScri(CkMigrateMessage* /*unused*/) {}

  CauchySecondOrderCleanScri(double angular_coordinate_tolerance,
                             size_t max_iterations, bool require_convergence,
                             double max_scri_second_derivative);

  CauchySecondOrderCleanScri() = default;
