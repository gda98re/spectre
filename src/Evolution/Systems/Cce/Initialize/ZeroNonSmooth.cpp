...
ZeroNonSmooth<false>::ZeroNonSmooth(const double angular_coordinate_tolerance,
...
std::unique_ptr<InitializeJ<false>> ZeroNonSmooth<false>::get_clone() const {
  return std::make_unique<ZeroNonSmooth>(angular_coordinate_tolerance_,
                                         max_iterations_);
}

void ZeroNonSmooth<false>::operator()(
...
PUP::able::PUP_ID ZeroNonSmooth<false>::my_PUP_ID = 0;  // NOLINT

ZeroNonSmooth<true>::ZeroNonSmooth(const double angular_coordinate_tolerance,
                                   const size_t max_iterations,
                                   const bool require_convergence)
    : angular_coordinate_tolerance_{angular_coordinate_tolerance},
      max_iterations_{max_iterations},
...
std::unique_ptr<InitializeJ<true>> ZeroNonSmooth<true>::get_clone() const {
  return std::make_unique<ZeroNonSmooth>(angular_coordinate_tolerance_,
                                         max_iterations_);
}

void ZeroNonSmooth<true>::operator()(
    const gsl::not_null<Scalar<SpinWeighted<ComplexDataVector, 2>>*> j,
    const gsl::not_null<tnsr::i<DataVector, 3>*> cartesian_cauchy_coordinates,
    const gsl::not_null<
        tnsr::i<DataVector, 2, ::Frame::Spherical<::Frame::Inertial>>*>
  ...
  auto finalize_function =
      [&l_max, &target_c_inv, &target_d_inv](
          const Scalar<SpinWeighted<ComplexDataVector, 2>>& gauge_c,
          const Scalar<SpinWeighted<ComplexDataVector, 0>>& gauge_d,
...
PUP::able::PUP_ID ZeroNonSmooth<true>::my_PUP_ID = 0;  // NOLINT
}  // namespace Cce::InitializeJ
