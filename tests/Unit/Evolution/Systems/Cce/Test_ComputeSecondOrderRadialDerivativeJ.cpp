...
void test_h_equation_is_satisfied(const size_t l_max) {
  ...
  const auto dr_j = random_boundary_scalar<2>(
      make_not_null(&generator), make_not_null(&coefficient_distribution),
      l_max);
  const auto du_dy_j = random_boundary_scalar<2>(
      make_not_null(&generator), make_not_null(&coefficient_distribution),
      l_max);
  ...
  // `du_dy_j` is already the numerical-coordinate Dy of h_numerical.
  ...
  compute_dy_dy_j(make_not_null(&dy_dy_j), j, u, w, beta, q, h, dr_j, du_dy_j,
                  du_r, r, l_max);

  // The natural scale of the H equation is the size of its source term, the
  // residual evaluated with dy^2 J set to zero.
  const auto residual_without_dy_dy_j =
      evaluate_worldtube_h_residual(zero_dy_dy_j, j, u, w, beta, q, dy_j,
                                    h_numerical, du_dy_j, du_r, r, l_max);
  double scale = 0.0;
  for (const auto entry : get(residual_without_dy_dy_j).data()) {
    scale = std::max(scale, std::abs(entry));
  }

  const auto residual_at_solution =
      evaluate_worldtube_h_residual(get(dy_dy_j).data(), j, u, w, beta, q, dy_j,
                                    h_numerical, du_dy_j, du_r, r, l_max);
