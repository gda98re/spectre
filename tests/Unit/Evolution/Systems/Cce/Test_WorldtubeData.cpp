...
void test_bondi_data_manager_du_dr_j_interpolator() {
  ...
  tmpl::for_each<
      ...
      [&shared = shared, &substituted = substituted](auto tag_v) {
        using tag = tmpl::type_from<decltype(tag_v)>;
        if constexpr (std::is_same_v<tag, Tags::BoundaryValue<Tags::Du<
                                              Tags::Dr<Tags::BondiJ>>>>) {
          CHECK(max(abs(get(get<tag>(shared)).data() -
                        get(get<tag>(substituted)).data())) > 0.0);
        } else {
          CHECK_ITERABLE_APPROX(get(get<tag>(shared)).data(),
                                get(get<tag>(substituted)).data());
        }
      });
